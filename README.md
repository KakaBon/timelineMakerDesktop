...
- 年-月-日：`2014-09-25`、`2014/09/25`、`2014.09.25`
- 中文年月日：`2014年9月25日`
- 日-月-年：`25.09.2014`、`25/09/2014`、`25-09-2014`
- 月-日-年：`09.25.2014`、`09/25/2014`、`09-25-2014`
- 月份名称或缩写：`25 Sep 2014`、`Sep 25, 2014`、`2014 Sep 25`
- 英文月份全称同样支持，例如 `25 September 2014`、`September 25, 2014`
- 常见德文月份名称和缩写同样支持，例如 `25 Sept 2014`、`25 Oktober 2014`、`25 Dez 2014`

月份名称写法可使用空格、点号、斜杠或连字符分隔，例如 `25-Sep-2014`、`25.Sep.2014` 也可以读取。

对于纯数字的 `DD.MM.YYYY` / `MM.DD.YYYY`、`DD/MM/YYYY` / `MM/DD/YYYY`、`DD-MM-YYYY` / `MM-DD-YYYY`：

- 如果某一行本身就能唯一判断顺序，例如 `25.09.2014`，程序会据此判断同一日期列的整体顺序；
- 因此同一列中的 `09.08.2015` 会沿用已经确认的日/月顺序；
- 如果整列全部都是 `09.08.2015` 这种日和月都不超过 12、无法判断顺序的日期，程序不会猜测，会要求改成 `YYYY-MM-DD`、中文年月日或月份名称格式。

同一份 CSV 的日期格式应保持一致，尤其不要混用不同的日/月顺序或不同数字分隔符。
